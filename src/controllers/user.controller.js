...
const generateAcessAndRefreshTokens = async (userId) => {
  try {
    console.log("generateAcessAndRefreshTokens called with userId:", userId);
    ...
    console.error("Token generation error stack:", error.stack);
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (incomingRefreshToken !== user?.refreshToken) {
      throw new ApiError(401, "Refresh token is expired or  ");
    }
    const options = {
      httpOnly: true,
      secure: true,
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  const user = await User.findByIdAndUpdate(
    ...
    {
      $set: {
        coverImage: coverImagecoverImage.url,
      },
    },
    { new: true }
  ).select("-password");
